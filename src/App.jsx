    ...
    <div style={{ width: '100%', height: '100%', position: 'relative', background: '#0a0015', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', bottom: '100px', left: '16px', right: '80px', color: 'white' }}>
        <div style={{
          ...
          letterSpacing: '0.15em', textTransform: 'uppercase', color: '#c4b5fd', marginBottom: '8px',
      ...
      {floatingEmoji && (
        <div style={{
          ...
          fontSize: '40px', animation: 'fadeUp 1s ease forwards', pointerEvents: 'none',
        }}>{floatingEmoji}</div>
      )}
      ...
      {showComments && (
        <div style={{
          ...
          background: 'rgba(10,0,21,0.97)', border: '1px solid rgba(139,92,246,0.2)',
          borderRadius: '24px 24px 0 0', padding: '16px',
          display: 'flex', flexDirection: 'column', backdropFilter: 'blur(20px)',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <h4 style={{ color: '#c4b5fd', fontSize: '14px', fontWeight: '700' }}>💬 Comments ({comments.length})</h4>
            <button onClick={() => setShowComments(false)} style={{
              background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: '20px', cursor: 'pointer',
            }}>×</button>
          </div>

          <div style={{ flex: 1, overflowY: 'auto', marginBottom: '12px', gap: '10px', display: 'flex', flexDirection: 'column' }}>
            {comments.length === 0 && (
            ...
            {comments.map(c => (
              <div key={c.id} style={{
                ...
                <p style={{ color: '#a78bfa', fontSize: '11px', fontWeight: '700', marginBottom: '4px' }}>@{c.username}</p>
                <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '13px', lineHeight: 1.4 }}>{c.comment}</p>
              </div>
            ))}
          </div>
          ...
          <input
            ...
            style={{
              ...
              fontSize: '12px', marginBottom: '8px', outline: 'none',
            }}
          />
          <div style={{ display: 'flex', gap: '8px' }}>
            <input
              ...
              style={{
                ...
                padding: '10px 12px', color: 'white', fontSize: '13px', outline: 'none',
              }}
            />
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {/* Centered phone column — full width on mobile, 420px on PC */}
      <div style={{
        position: 'relative',
        width: '100%',
        maxWidth: '420px',
